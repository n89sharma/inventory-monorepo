...
export const storePartTableColumns: ColumnDef<StorePartSummary>[] = [
  ...
  { accessorKey: "warehouse_code", header: "Warehouse", size: 90 },
  {
    ...
    cell: ({ row }) => (
      <div className="text-center tabular-nums">{row.original.on_hand}</div>
    ),
  },
  {
...
export const storeTransactionLedgerColumns: ColumnDef<StoreTransactionRow>[] = [
  {
    ...
    size: 120,
    cell: ({ row }) => (
      <span className="font-mono">{row.original.store_transaction_number}</span>
    ),
  },
  {
  ...
  },
  {
    ...
    size: 100,
    cell: ({ row }) =>
      row.original.unit_cost === null ? '—' : formatUSD(row.original.unit_cost),
  },
  {
    id: "destination",
    header: "Destination",
