...
export function SearchPopoverContent({
  ...
  if (!hasResults) return <p className="py-6 text-center text-muted-foreground">No results</p>
  ...
  return (
    <Tabs value={activeTab} onValueChange={onTabChange}>
      <TabsList className="w-full">
        ...
        <TabsTrigger
          value="arrivals"
          disabled={results.arrivals.length === 0}
          className="cursor-pointer"
        >
          Arrivals
        </TabsTrigger>
        <TabsTrigger
          value="departures"
          disabled={results.departures.length === 0}
          className="cursor-pointer"
        >
          Departures
        </TabsTrigger>
        <TabsTrigger
          value="transfers"
          disabled={results.transfers.length === 0}
          className="cursor-pointer"
        >
          Transfers
        </TabsTrigger>
        <TabsTrigger value="holds" disabled={results.holds.length === 0} className="cursor-pointer">
          Holds
        </TabsTrigger>
        <TabsTrigger
          value="invoices"
          disabled={results.invoices.length === 0}
          className="cursor-pointer"
        >
          Invoices
        </TabsTrigger>
