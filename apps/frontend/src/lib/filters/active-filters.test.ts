...
const SCOPE_GROUPS = [['wh'], ['instock'], ['brand']] as const satisfies FilterParamGroups

describe('countActiveFilterGroups', () => {
  it('counts nothing when every param is absent', () => {
    expect(countActiveFilterGroups(SCOPE_GROUPS, { wh: null, instock: null, brand: null })).toBe(0)
  })
  ...
  it('counts each populated control separately', () => {
    expect(countActiveFilterGroups(SCOPE_GROUPS, { wh: [1], instock: true, brand: 7 })).toBe(3)
  })
  ...
  it('ignores a flag that is off', () => {
    expect(countActiveFilterGroups([['instock']], { instock: false })).toBe(0)
    expect(countActiveFilterGroups([['instock']], { instock: true })).toBe(1)
  })
