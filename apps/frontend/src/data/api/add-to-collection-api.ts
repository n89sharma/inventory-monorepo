...
export async function getCollectionAssets(target: CollectionTarget): Promise<AssetSummary[]> {
  switch (target.kind) {
    case 'departure':
      return (await getDepartureDetail(target.number)).assets
    case 'transfer':
      return (await getTransferDetail(target.number)).assets
    case 'hold':
      return (await getHoldDetail(target.number)).assets
    case 'invoice':
      return (await getInvoiceDetail(target.number)).assets
  }
}
