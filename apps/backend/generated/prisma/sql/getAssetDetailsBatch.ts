
/* !!! This is code generated by Prisma. Do not edit directly. !!! */
/* eslint-disable */
// biome-ignore-all lint: generated file
// @ts-nocheck 
import * as $runtime from "@prisma/client/runtime/client"

/**
 * @param _text
 */
export const getAssetDetailsBatch = $runtime.makeTypedQueryFactory("select\na.id as id,\na.barcode as barcode,\na.serial_number as serial_number,\nm.\"name\" as model,\nm.is_colour as is_colour,\nb.\"name\" as brand,\nb.id as brand_id,\nat.asset_type as asset_type,\ns.status  as status,\nrd.status  as readiness,\nl.warehouse_id as warehouse_id,\nw.city_code as warehouse_code,\nw.street as warehouse_street,\nz.zone as zone,\nl.bin as bin,\na.created_at as created_at,\na.is_in_transit as is_in_transit,\nco.\"name\" as country_of_origin,\na.country_of_origin_id as country_of_origin_id,\na.manufactured_year as manufactured_year,\nm.weight as weight,\nm.size as size,\nc.purchase_cost as purchase_cost,\nc.transport_cost as transport_cost,\nc.processing_cost as processing_cost,\nc.other_cost as other_cost,\nc.parts_cost as parts_cost,\nc.total_cost as total_cost,\nc.sale_price as sale_price,\nts.cassettes as ts_cassettes,\ncmp.\"name\" as internal_finisher,\nts.component_id as internal_finisher_id,\nts.meter_black as meter_black,\nts.meter_colour as meter_colour,\nts.meter_total as meter_total,\nts.drum_life_c as drum_life_c,\nts.drum_life_m as drum_life_m,\nts.drum_life_y as drum_life_y,\nts.drum_life_k as drum_life_k,\nts.toner_life_c as toner_life_c,\nts.toner_life_m as toner_life_m,\nts.toner_life_y as toner_life_y,\nts.toner_life_k as toner_life_k,\nhub.email as hold_by_email,\nhub.\"name\" as hold_by_name,\nhuf.email as hold_for_email,\nhuf.\"name\" as hold_for_name,\nh.created_at as hold_created_at,\nho.\"name\" as hold_customer,\nh.from_dt as hold_from,\nh.to_dt as hold_to,\nh.notes as hold_notes,\nh.hold_number as hold_number,\nr.arrival_number as arrival_number,\nrc.\"name\" as arrival_origin,\nrw.city_code as arrival_destination_city_code,\nrw.street as arrival_destination_street,\nrt.\"name\" as arrival_transporter,\nru.email as arrival_created_by_email,\nru.\"name\" as arrival_created_by_name,\nr.notes as arrival_notes,\nr.created_at as arrival_created_at,\nd.departure_number as departure_number,\ndw.city_code as departure_origin_city_code,\ndw.street as departure_origin_street,\ndc.\"name\" as departure_destination,\ndt.\"name\" as departure_transporter,\ndu.email as departure_created_by_email,\ndu.\"name\" as departure_created_by_name,\nd.notes as departure_notes,\nd.created_at as departure_created_at,\npi.invoice_number as purchase_invoice_number,\npi.is_cleared as purchase_invoice_is_cleared,\nsi.invoice_number as sales_invoice_number,\nsi.is_cleared as sales_invoice_is_cleared,\nlc.comment as latest_comment\nfrom \"Asset\" a\njoin \"Model\" m on m.id = a.model_id\njoin \"Brand\" b on b.id = m.brand_id\njoin \"AssetType\" at on at.id = m.asset_type_id\njoin \"Status\" s on s.id = a.status_id\njoin \"Readiness\" rd on rd.id = a.readiness_id\nleft join \"Cost\" c on c.asset_id = a.id\nleft join \"TechnicalSpecification\" ts on ts.asset_id = a.id\nleft join \"Component\" cmp on cmp.id = ts.component_id\nleft join \"Location\" l on l.id = a.location_id\nleft join \"Warehouse\" w on w.id = l.warehouse_id\nleft join \"Zone\" z on z.id = l.zone_id\nleft join \"Hold\" h on h.id = a.hold_id\nleft join \"User\" hub on hub.id = h.created_by_id\nleft join \"User\" huf on huf.id = h.created_for_id\nleft join \"Organization\" ho on ho.id = h.customer_id\nleft join \"Arrival\" r on r.id = a.arrival_id\nleft join \"User\" ru on ru.id = r.created_by_id\nleft join \"Organization\" rc on rc.id = r.origin_id\nleft join \"Organization\" rt on rt.id = r.transporter_id\nleft join \"Warehouse\" rw on rw.id = r.destination_id\nleft join \"Departure\" d on d.id = a.departure_id\nleft join \"User\" du on du.id = d.created_by_id\nleft join \"Warehouse\" dw on dw.id = d.origin_id\nleft join \"Organization\" dt on dt.id = d.transporter_id\nleft join \"Organization\" dc on dc.id = d.destination_id\nleft join \"Invoice\" pi on pi.id = a.purchase_invoice_id\nleft join \"Invoice\" si on si.id = a.sales_invoice_id\nleft join \"Country\" co on co.id = a.country_of_origin_id\nleft join lateral (\nselect cm.comment\nfrom \"Comment\" cm\nwhere cm.asset_id = a.id\norder by cm.created_at desc\nlimit 1\n) lc on true\nwhere a.barcode = ANY($1::text[])") as (_text: string[]) => $runtime.TypedSql<getAssetDetailsBatch.Parameters, getAssetDetailsBatch.Result>

export namespace getAssetDetailsBatch {
  export type Parameters = [_text: string[]]
  export type Result = {
    id: number
    barcode: string
    serial_number: string
    model: string
    is_colour: boolean
    brand: string
    brand_id: number
    asset_type: string
    status: string
    readiness: string
    warehouse_id: number | null
    warehouse_code: string | null
    warehouse_street: string | null
    zone: string | null
    bin: string | null
    created_at: Date
    is_in_transit: boolean
    country_of_origin: string | null
    country_of_origin_id: number | null
    manufactured_year: number | null
    weight: number
    size: number
    purchase_cost: $runtime.Decimal | null
    transport_cost: $runtime.Decimal | null
    processing_cost: $runtime.Decimal | null
    other_cost: $runtime.Decimal | null
    parts_cost: $runtime.Decimal | null
    total_cost: $runtime.Decimal | null
    sale_price: $runtime.Decimal | null
    ts_cassettes: number | null
    internal_finisher: string | null
    internal_finisher_id: number | null
    meter_black: number | null
    meter_colour: number | null
    meter_total: number | null
    drum_life_c: number | null
    drum_life_m: number | null
    drum_life_y: number | null
    drum_life_k: number | null
    toner_life_c: number | null
    toner_life_m: number | null
    toner_life_y: number | null
    toner_life_k: number | null
    hold_by_email: string | null
    hold_by_name: string | null
    hold_for_email: string | null
    hold_for_name: string | null
    hold_created_at: Date | null
    hold_customer: string | null
    hold_from: Date | null
    hold_to: Date | null
    hold_notes: string | null
    hold_number: string | null
    arrival_number: string | null
    arrival_origin: string | null
    arrival_destination_city_code: string | null
    arrival_destination_street: string | null
    arrival_transporter: string | null
    arrival_created_by_email: string | null
    arrival_created_by_name: string | null
    arrival_notes: string | null
    arrival_created_at: Date | null
    departure_number: string | null
    departure_origin_city_code: string | null
    departure_origin_street: string | null
    departure_destination: string | null
    departure_transporter: string | null
    departure_created_by_email: string | null
    departure_created_by_name: string | null
    departure_notes: string | null
    departure_created_at: Date | null
    purchase_invoice_number: string | null
    purchase_invoice_is_cleared: boolean | null
    sales_invoice_number: string | null
    sales_invoice_is_cleared: boolean | null
    latest_comment: string | null
  }
}
