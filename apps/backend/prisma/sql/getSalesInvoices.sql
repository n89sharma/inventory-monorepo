  ...
  where it.type = 'SALE'
    ...
    and not exists (
      ...
      where a.sales_invoice_id = i.id
        and a.departure_id is not null
    )
)
select
  i.id as id,
  ...
  mv.departure_start_date as departure_start_date,
  mv.departure_end_date as departure_end_date,
...
from "Invoice" i
  ...
  left join lateral (
    ...
    where ast.sales_invoice_id = i.id
  ) ac on true
  left join lateral (
    select
  ...
  left join lateral (
    select
      min(d.created_at) as departure_start_date,
      max(d.created_at) as departure_end_date
    from "Asset" a
...
where it.type = 'SALE'
order by coalesce(mv.departure_start_date, i.invoice_date::timestamp) desc
limit 500
